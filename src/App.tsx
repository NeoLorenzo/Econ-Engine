...
export default function App() {
  const [draft, setDraft] = useState({ firmStarts: DEFAULT_FIRM_START_DRAFT, step: '1.00', dailySupply: '10', seed: String(DEFAULT_SEED), transportRate: '0.02', expenditureBase: '50.00' })
  ...
  const reset = () => {
    ...
    const config: SimulationConfig = { startingPriceCents: 200, firmStartingPricesCents, initialStepCents: Math.max(1, Math.round(Number(draft.step || 0) * 100)), dailySupplyPerIndustry: Math.max(0, Math.round(Number(draft.dailySupply || 0))), seed: Math.round(Number(draft.seed || DEFAULT_SEED)), transportCostPerTileCents: Math.max(0, Math.round(Number(draft.transportRate || 0) * 100)), dailyExpenditureBudgetCents: Math.max(0, Math.round(Number(draft.expenditureBase || 0) * 100)) }
  ...
  return <main>
    <header className="hero"><div className="eyebrow">MVP 4 · Full Spatial Competition</div><div className="hero-row"><div><h1>Econ<span>—</span>Engine</h1><p>Four competitive consumer industries combine seeded geography, delivered-cost choice, and percentage expenditure constraints.</p></div><div className="system-note"><i />Deterministic · $500 closed circuit</div></div></header>
    ...
    <section className="baseline panel"><div><span>Spatial world</span><strong>{state.config.gridWidth} × {state.config.gridHeight} tiles</strong></div><div><span>Consumer supply</span><strong>{state.config.dailySupplyPerIndustry} per firm</strong></div><div><span>Travel rate</span><strong>{money(state.config.transportCostPerTileCents!)} / tile</strong></div><div><span>Parity target</span><strong>$50 per household</strong></div></section>
    <BudgetModel state={state} />
    <section className="metrics-grid"><Metric label="Current day" value={String(state.day)} detail={running ? 'Running' : 'Paused'} /><Metric label="Transport revenue" value={money(latest?.totalTransportRevenueCents ?? 0)} detail={`${latest?.entertainmentTrips ?? 0} trips · ${latest?.totalTilesTravelled ?? 0} tiles`} /><Metric label="Pre-parity Gini" value={(latest?.householdCashGiniBeforeParity ?? 0).toFixed(3)} detail="Geographic spending" /><Metric label="Post-parity Gini" value={(latest?.householdCashGini ?? 0).toFixed(3)} detail="Explicit transfers" /><Metric label="Total money" value={money(latest?.totalMoneyCents ?? TOTAL_MONEY_CENTS)} detail="✓ Exact closed circuit" accent /><Metric label="Locally settled" value={`${settledCount} / 8`} detail={`Seed ${state.config.seed}`} accent={settledCount === 8} /></section>
    ...
    <section className="trajectory-section">
      <div className="trajectory-heading"><h2>Profit trajectories</h2><p>One graph per consumer industry</p></div>

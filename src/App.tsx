...
function PriceChart({ state }: { state: SimulationState }) {
  ...
  return <ResponsiveContainer width="100%" height="100%"><LineChart data={data} margin={{ top: 10, right: 12, left: -16, bottom: 0 }}><CartesianGrid stroke="#202825" vertical={false} /><XAxis dataKey="day" stroke="#69756f" tickLine={false} axisLine={false} /><YAxis stroke="#69756f" tickFormatter={(v) => `$${v}`} tickLine={false} axisLine={false} /><Tooltip contentStyle={chartTooltip} formatter={(v) => money(Number(v ?? 0) * 100)} labelFormatter={(v) => `Day ${v}`} /><Line type="monotone" dataKey="posted" name="Posted price" stroke="#deff75" dot={false} strokeWidth={2} /><Line type="stepAfter" dataKey="best" name="Best known" stroke="#65bfa1" dot={false} strokeWidth={1.5} strokeDasharray="5 5" /></LineChart></ResponsiveContainer>
}

function SimpleChart({ state, field, color, formatter = (v) => String(v) }: { state: SimulationState; field: 'preTaxProfitCents' | 'unitsSold' | 'totalMoneyCents'; color: string; formatter?: (value: number) => string }) {
  const data = state.metrics.map((m) => ({ day: m.day, value: m[field] }))
  return <ResponsiveContainer width="100%" height="100%"><LineChart data={data} margin={{ top: 10, right: 12, left: -16, bottom: 0 }}><CartesianGrid stroke="#202825" vertical={false} /><XAxis dataKey="day" stroke="#69756f" tickLine={false} axisLine={false} /><YAxis stroke="#69756f" tickFormatter={formatter} domain={field === 'totalMoneyCents' ? [9900, 10100] : ['auto', 'auto']} tickLine={false} axisLine={false} /><Tooltip contentStyle={chartTooltip} formatter={(v) => formatter(Number(v ?? 0))} labelFormatter={(v) => `Day ${v}`} /><Line type="monotone" dataKey="value" name={field === 'unitsSold' ? 'Units' : field === 'totalMoneyCents' ? 'Total money' : 'Profit'} stroke={color} dot={false} strokeWidth={2} /></LineChart></ResponsiveContainer>
}
...
export default function App() {
  ...
  const recentEvents = useMemo(() => [...state.events].reverse().slice(0, 28), [state.events])
  ...
  return <main>
    ...
    <section className="metrics-grid">
      ...
      <Metric label="Posted price" value={money(state.firm.postedPriceCents)} detail="Next market day" />
      ...
      <Metric label="Units sold" value={`${latest?.unitsSold ?? 0} / 10`} detail="Most recent day" />
      <Metric label="Daily profit" value={money(latest?.preTaxProfitCents ?? 0)} detail="Equals revenue; zero cost" />
      <Metric label="Total money" value={money(latest?.totalMoneyCents ?? 10_000)} detail="Invariant: exactly $100" />
    ...
    <section className="decision panel"><div className="decision-mark">↗</div><div><span>Latest pricing decision</span><p>{state.latestDecisionReason}</p></div></section>

    <section className="charts-grid">
      <ChartPanel title="Price discovery" subtitle="Posted price versus the firm’s best realized price"><PriceChart state={state} /></ChartPanel>
      ...
      <ChartPanel title="Money conservation" subtitle="Total cash remains exactly $100.00"><SimpleChart state={state} field="totalMoneyCents" color="#97a3ff" formatter={money} /></ChartPanel>
    </section>
    ...
    <section className="inspection-grid">
      ...
      <div className="panel ledger"><div className="panel-heading"><div><h2>Recent event ledger</h2><p>Explicit causes, newest first</p></div><span>{Math.min(state.events.length, 28)} shown</span></div><div className="event-list">{recentEvents.length === 0 ? <div className="empty">Run or step the simulation to inspect its causal ledger.</div> : recentEvents.map((event) => <div className="event" key={event.id}><span>D{event.day}</span><div><strong>{event.type.replaceAll('_', ' ')}</strong><p>{event.description}</p></div></div>)}</div></div>
    </section>

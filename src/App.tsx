...
export default function App() {
  const [draft, setDraft] = useState({ firmStarts: DEFAULT_FIRM_START_DRAFT, step: '1.00', dailySupply: '10' })
  ...
  const reset = () => {
    ...
    const config: SimulationConfig = { startingPriceCents: 200, firmStartingPricesCents, initialStepCents: Math.max(1, Math.round(Number(draft.step || 0) * 100)), dailySupplyPerIndustry: Math.max(0, Math.round(Number(draft.dailySupply || 0))) }
  ...
  return <main>
    ...
    <section className="panel experiment-panel"><div className="panel-heading"><div><h2>Competition experiment</h2><p>Entertainment A starts $1; B starts $8; controls retain varied starts</p></div><button onClick={() => setExperiment(runMultiIndustryExperiment())}>{experiment ? 'Run again' : 'Run experiment'}</button></div>{experiment ? <div className="market-table-wrap"><table><thead><tr><th>Firm</th><th>Start</th><th>Final price</th><th>Converged best</th><th>Day</th><th>Final sales</th><th>Share</th></tr></thead><tbody>{experiment.firms.map((firm) => <tr key={firm.firmId}><td>{firm.firmId}</td><td>{money(firm.startingPriceCents)}</td><td>{money(firm.finalPriceCents)}</td><td>{firm.convergedPriceCents === null ? 'No convergence' : money(firm.convergedPriceCents)}</td><td>{firm.daysToConvergence ?? `>${experiment.horizonDays}`}</td><td>{firm.finalUnitsSold}</td><td>{(firm.finalMarketShare * 100).toFixed(0)}%</td></tr>)}</tbody></table><p className="experiment-result">Final cash {money(experiment.finalHouseholdCashMinimumCents)} / {money(experiment.finalHouseholdCashMedianCents)} / {money(experiment.finalHouseholdCashMaximumCents)} · Gini {experiment.finalHouseholdCashGini.toFixed(3)} · total money {money(experiment.totalMoneyCents)}</p></div> : <div className="experiment-empty">Entertainment competitors learn only from their own realised sales and profit. No competitive endpoint is encoded or assumed.</div>}</section>
    <section className="panel experiment-panel grid-experiment"><div className="panel-heading"><div><h2>Starting-price sensitivity</h2><p>64 deterministic A/B combinations · cell = A/B learner endpoint</p></div><button onClick={() => setCompetitionGrid(runCompetitionStartingPriceGrid())}>{competitionGrid ? 'Run grid again' : 'Run 8×8 grid'}</button></div>{competitionGrid ? <div className="competition-matrix-wrap"><table><thead><tr><th>A \ B</th>{competitionGrid.startingPricesCents.map((start) => <th key={start}>{money(start)}</th>)}</tr></thead><tbody>{competitionGrid.startingPricesCents.map((aStart) => <tr key={aStart}><th>{money(aStart)}</th>{competitionGrid.startingPricesCents.map((bStart) => { const result = competitionGrid.results.find((item) => item.firmAStartCents === aStart && item.firmBStartCents === bStart)!; return <td key={bStart} title={`A day ${result.firmAConvergenceDay ?? '—'} · B day ${result.firmBConvergenceDay ?? '—'}`}>{result.bothConverged ? `${money(result.firmAEndpointCents!)} / ${money(result.firmBEndpointCents!)}` : `Not converged ≤ D${competitionGrid.horizonDays}`}</td> })}</tr>)}</tbody></table></div> : <div className="experiment-empty">Runs the full $1, $2, $3, $4, $5, $6, $8, $10 Cartesian grid with a deterministic 300-day horizon. Controls and all institutions remain fixed.</div>}</section>

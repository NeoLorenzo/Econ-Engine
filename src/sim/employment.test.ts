...
describe('[MVP5-Employment-007]', () => {
  it('assigns exactly one fixed seeded job per household with canonical slot counts', () => {
    ...
    expect(new Set(first.firms.flatMap(({ employeeIds }) => employeeIds)).size).toBe(100)
  ...
  it('produces explicitly from labor, expires goods, and excludes Transport production', () => {
    ...
    day.firms.filter(({ industryId }) => industryId !== 'transport').forEach((firm) => { expect(firm.unitsProducedToday).toBe(50); expect(firm.unitsSoldToday + firm.unitsExpiredToday).toBe(50) })
  ...
  it('keeps Government inactive, permits divergence, and conserves the closed circuit long-run', () => {
    ...
    expect(state.government.cashCents).toBe(0); expect(totalMoney(state)).toBe(500_000)

...
describe('deterministic multi-industry experiment', () => {
  it('preserves control endpoints and records competitive trajectories reproducibly', () => {
    ...
    expect(first.firms.filter(({ industryId }) => !['entertainment', 'transport'].includes(industryId)).every((firm) => firm.convergedPriceCents === EXPECTED_INDUSTRY_OPTIMA_CENTS[firm.industryId] && firm.daysToConvergence !== null)).toBe(true)

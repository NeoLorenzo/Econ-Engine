...
Every material action emits a typed event. Events distinguish causal flows from current state and make individual transfers auditable. Daily metrics are measurements derived from underlying state. Both histories are bounded in memory.

## Interface boundary

React owns controls, clock scheduling and presentation. A step calls the pure engine and renders the returned snapshot. Charts read metrics; the ledger reads events; agent tables read agent balances. No purchase, tax, redistribution or price-learning rule exists in a component.
